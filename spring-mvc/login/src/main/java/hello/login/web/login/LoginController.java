...
//    @PostMapping("/login")
    public String loginV3(@ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        HttpSession session = request.getSession(); // 세션이 있으면 있는 세션 반환, 없으성 신규 세션 생성
        session.setAttribute(SessionConst.LOGIN_MEMBER, member); // 세션에 로그인 회원 정보 저장

        return "redirect:/";
    }

    @PostMapping("/login")
    public String loginV4(@ModelAttribute LoginForm form, @RequestParam(defaultValue = "/") String redirectURL, BindingResult bindingResult, HttpServletRequest request) {
        if (bindingResult.hasErrors()) {
            return "login/loginForm";
        }
